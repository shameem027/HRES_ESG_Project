...
# Normalize weights for the API call
    ...
    if 'recommendation' in st.session_state and st.session_state.recommendation:
        ...
        with v1:
            ...
            df_financial = pd.DataFrame(
                {'Metric': ["Total Project Cost", "Annual Maintenance", "Annual Savings", "Payback Period (Years)"],
                 'Value': [format_currency(res['total_cost']), format_currency(res['annual_maintenance_cost_eur']),
                           format_currency(res['annual_savings_eur']), f"{res['payback_period_years']:.1f}"]})
        ...
        with e_kpis:
            with st.expander("🌳 Environmental KPIs", expanded=True):
                st.table(pd.DataFrame({'KPI': ['CO2 Reduction (tons/yr)', 'Land Use (sqm)', 'Water Savings (m3/yr)',
                                               'Energy Waste (%)', 'Degradation Rate'],
                                       'Value': [format_number(kpis.get('env_co2_reduction_tons_yr', 0)),
                                                 ...
                                                 f"{kpis.get('env_degradation_rate', 0):.3f}"]}))
        with s_kpis:
            with st.expander("🤝 Social KPIs", expanded=True):
                st.table(pd.DataFrame({'KPI': ['Local Jobs Created', 'Energy Resilience (hrs)',
                                               'Grid Strain Reduction (%)', 'Community Investment (€)'],
                                       'Value': [f"{kpis.get('soc_local_jobs_fte', 0):.2f}",
                                                 ...
                                                 format_currency(kpis.get('soc_community_investment_eur', 0))]}))
        with g_kpis:
...
with tab_predictor:
    ...
    st.warning(
        "️️️⚠️ **Prerequisite:** This feature requires the ML models to be trained. Please go to the **Airflow UI** (`http://localhost:8080`), un-pause the `HRES_PhD_Automation_Pipeline` DAG, and trigger a manual run. This tab will work after the DAG completes successfully.")
...
with tab_advisor:
    ...
    # Using a session state variable to handle button clicks for prompts
    if c1.button(
    ...
    # **FIX**: Use st.text_input and a button INSIDE the tab for chat
    prompt = st.text_input("Your request:", value=st.session_state.get('prompt_text', ''), key="chat_input")

    if st.button("Send to Advisor", key="send_chat"):
        if prompt:
            ...
            st.session_state.prompt_text = ""  # Clear the prompt box after sending
            st.rerun()
...
with tab_about:
    ...
    st.subheader("I. Project Description & Business Value (15%)")
    st.markdown("""
    - **Project Goal:** To create an intelligent decision-support tool ("BonsAI") that de-risks investment in Hybrid Renewable Energy Systems (HRES) by providing data-driven, ESG-aligned recommendations.
    - **Business Problem:** Selecting the right HRES is a high-stakes decision, forcing organizations to balance large capital expenditures against long-term savings and complex sustainability goals. This process is typically slow, expensive, and requires specialist consultants.
    - **Customer Value:** Our app empowers facility managers and sustainability officers to instantly model scenarios and explore trade-offs between cost and ESG factors. It translates vague objectives (e.g., "becoming greener") into concrete, financially viable system designs, dramatically accelerating the path to sustainability and reducing consultancy costs.
    """)

    st.subheader("II. MLOps Workflow & Technical Implementation (60%)")
    st.markdown("""
    This is a fully functioning, containerized application demonstrating a complete MLOps lifecycle. The workflow is as follows:
    1.  **Data Generation:** A Python script (`HRES_Dataset_Generator.py`) simulates thousands of HRES configurations to create a rich, proprietary dataset.
    2.  **Model Training & Registration:** An `HRES_ML_Model.py` script trains a Random Forest model on this data. Using **MLflow**, the training run, parameters, metrics, and the final model artifact are logged. The model is then versioned and registered in the **MLflow Model Registry**, creating a governed, single source of truth.
    3.  **Automated Pipeline (Airflow):** An **Apache Airflow** pipeline (`HRES_Automation_Pipeline.py`) orchestrates the entire MLOps lifecycle. It automatically triggers the data generation and model retraining on a schedule, ensuring the system continuously improves.
    4.  **API Deployment (Flask):** A robust Flask API (`recommender_api.py`) serves both the core MCDA decision logic and the latest registered ML model from MLflow. It also integrates with Azure OpenAI to power the AI Advisor.
    5.  **Containerization (Docker):** Every service (API, UI, Airflow, MLflow, Database) is individually containerized with **Docker** and orchestrated with **Docker Compose**, guaranteeing a consistent, portable, and scalable environment.
    6.  **User Interface (Streamlit):** This interactive UI provides the user-facing dashboard for the entire system.
    """)
