# --- File: ui.py (Definitive Final Version with Auto-Startup Logic) ---
...
@st.cache_data(ttl=15)  # Cache the status check for 15 seconds
...
if not status:
    ...
    st.warning("Connecting to the backend services... Please wait, this may take a moment after starting the system.",
               icon="⏳")
    time.sleep(10)
    st.rerun()

elif not status.get("decision_engine_loaded", False):
    st.title("💡 HRES ESG Recommender System")
    st.info("""
    **System Initializing: Step 1 of 2**

    The backend is online, but the dataset has not been generated yet.

    The fully automated **Airflow pipeline** has been triggered in the background to create the dataset. This is a one-time process on first startup and may take 5-10 minutes.

    This page will automatically refresh and proceed to the next step once the data is ready.

    *You can monitor the live progress in the Airflow UI at [http://localhost:8080](http://localhost:8080) (user: airflow, pass: airflow).*
    """, icon="⚙️")
    with st.spinner("Waiting for dataset generation..."):
        time.sleep(30)
    st.rerun()

elif not status.get("ml_models_loaded", False):
    st.title("💡 HRES ESG Recommender System")
    st.info("""
    **System Initializing: Step 2 of 2**

    ✅ Dataset generation is complete!

    The Airflow pipeline is now training the Machine Learning models. This usually takes 1-2 minutes.

    This page will automatically refresh once the models are loaded and ready.
    """, icon="🤖")
    with st.spinner("Waiting for ML model training..."):
        time.sleep(20)
    st.rerun()

else:
    # --- SIDEBAR: Input Parameters (Only show if system is ready) ---
    with st.sidebar:
    ...
    # ... The rest of the UI code is identical to the previous "state-of-the-art" version ...
    # It will only be rendered once the 'else' block is entered.
    ...
    with tab_advisor:
        ...
        prompt = st.chat_input("Your request:", key="chat_input_main")
        if prompt:
