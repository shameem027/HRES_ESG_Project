...
4.  `annual_demand_kwh`: MUST be an integer. Use these defaults if not specified: Small_Office: 250000, Hospital: 1500000, University_Campus: 3000000.
...
4.  Do not invent any data not present in the `recommendation` context. Maintain a professional and encouraging tone.
...
def load_all_resources():
    # This hook ensures resources are loaded/checked before each request
    # This is more robust for an environment where services start at different times.
    ...
    if not decision_engine:
        try:
            ...
            logger.warning(f"⚠️ Decision Engine not yet loaded. Waiting for data. Error: {e}")
    ...
    if not ml_models:
        try:
            ...
            logger.warning(f"⚠️ Could not load ML models. Waiting for training. Error: {e}")
...
def health_check():
    # Health check now reflects the actual state of the loaded resources
    return jsonify({
        "status": "healthy",
...
def recommend():
    if not decision_engine: return jsonify(
        {"error": "Decision Engine not operational. Data may still be generating."}), 503
    try:
        req_data = RecommendRequest.model_validate(request.get_json())
        best_solution, status, _, _, pareto_df = decision_engine.run_full_pipeline(
...
def predict_ml():
    if not ml_models: return jsonify(
        {"error": "ML models not loaded. Please wait for the automated training pipeline to complete."}), 503
    try:
        ...
        if not encoder or req_data.scenario_name not in encoder: return jsonify(
            {"error": "Scenario not recognized by ML model"}), 400
...
def chat():
    ...
    try:
        ...
        if solution is None:
            ai_response = "Based on my interpretation of your request, our model could not find a feasible solution."
        else:
            context = {"user_query": user_query,
                       "recommendation": {"Total Project Cost": f"€{solution['total_cost']:,.0f}",
