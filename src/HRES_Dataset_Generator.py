...
INSTALLATION_OVERHEAD_FACTOR, ENGINEERING_CONSULTING_COST_FACTOR, PERMITTING_LEGAL_COST_FACTOR, OTHER_COMPONENTS_COST_FACTOR = 0.60, 0.15, 0.05, 0.10
...
# ... (Profile generation functions remain the same)
...
def generate_load_profile(annual_demand_kwh, profile_type='office'):
    profile_map = {'office': np.array([0.3] * 8 + [0.8] * 2 + [1.0] * 8 + [0.7] * 2 + [0.3] * 4),
                   ...
                   'data_center': np.array([0.95] * 24), 'industrial': np.array([0.4] * 7 + [1.0] * 10 + [0.4] * 7)}
...
def simulate_hres_yearly(config, load_profile, price_schedule, total_gen, solar_gen_profile, wind_gen_profile):
    ...
    for hour in range(HOURS_IN_YEAR):
        ...
        if net_power > 0:
            ...
            surplus, export_limit = net_power - charge_amount, (
                        config['num_solar_panels'] * 0.5 + config['num_wind_turbines'] * 1.0) * 0.5
    ...
    annual_amortized_battery_replacement_cost = (num_battery_replacements * config[
        'battery_kwh'] * COST_PER_BATTERY_KWH) / PROJECT_LIFETIME_YEARS

    # --- CRITICAL FINANCIAL FIX ---
    # The cost of financing the project must be included in the annual operating costs.
    annual_financing_cost = total_initial_capex * FINANCING_INTEREST_RATE

    # Corrected total operating cost calculation
    ...
    payback_period = total_initial_capex / (annual_savings + 1e-6)
    payback_period = np.clip(payback_period, 0,
                             PROJECT_LIFETIME_YEARS * 2) if annual_savings > 0 else PROJECT_LIFETIME_YEARS * 5  # Assign a very long payback if savings are negative
    ...
    return {
        ...
        'annual_kwh_curtailed': round(total_curtailment), 'wind_generation_kwh': round(wind_gen_profile.sum()),
        ...
        'annual_financing_cost_eur': round(annual_financing_cost)  # Also return this new value
    }


def main():
    # ... (main function orchestration remains the same, it is correct)
    ...
    base_configs = [{'num_solar_panels': s, 'num_wind_turbines': w, 'battery_kwh': b} for s in solar_options for w in
                    wind_options for b in battery_options if s > 0 or w > 0]
    for scenario_name, params in scenarios.items():
        ...
        for config_base in tqdm(base_configs, desc=f"Simulating for {scenario_name}"):
            ...
            initial_hardware_cost = (config['num_solar_panels'] * COST_PER_SOLAR_PANEL + config[
                'num_wind_turbines'] * COST_PER_WIND_TURBINE + config['battery_kwh'] * COST_PER_BATTERY_KWH)
            config['total_cost'] = initial_hardware_cost * (
                        1 + INSTALLATION_OVERHEAD_FACTOR + ENGINEERING_CONSULTING_COST_FACTOR + PERMITTING_LEGAL_COST_FACTOR + OTHER_COMPONENTS_COST_FACTOR)
            solar_gen, wind_gen = generate_solar_profile(config['num_solar_panels']) * (
                        1 - SOLAR_PANEL_DEGRADATION_PER_YEAR), generate_wind_profile(config['num_wind_turbines'])
            total_gen = solar_gen + wind_gen
            config.update(simulate_hres_yearly(config, load_profile, price_schedule, total_gen, solar_gen, wind_gen))
            # ESG KPIs
