...
def simulate_hres_yearly(config, load_profile, price_schedule, total_gen):
    ...
    # Calculate lifetime costs for battery replacement
    # Batteries replaced at year 8 and 16, so 2 replacements over the 25-year life
    ...
    payback_period = total_initial_capex / (annual_savings + 1e-9)  # Epsilon to prevent division by zero
    ...
    return {
        ...
        'annual_kwh_exported': round(total_grid_export), 'annual_kwh_curtailed': round(total_curtailment),
        'annual_maintenance_cost_eur': round(annual_om_cost), 'annual_financing_cost_eur': round(annual_financing_cost)
    }
...
def main():
    ...
    for scenario_name, params in scenarios.items():
        ...
        for config_base in tqdm(base_configs, desc=f"Simulating for {scenario_name}"):
            ...
            soft_costs = hardware_cost * (
                        INSTALLATION_COST_FACTOR + ENGINEERING_CONSULTING_COST_FACTOR + PERMITTING_LEGAL_COST_FACTOR + OTHER_COMPONENTS_COST_FACTOR)
            config['total_cost'] = hardware_cost + soft_costs
            ...
            config['gov_supply_chain_transparency_score'] = round(np.clip(
                (config['num_solar_panels'] * 8 + config['num_wind_turbines'] * 6) / (
                            config['num_solar_panels'] + config['num_wind_turbines'] + 1e-6), 1, 10), 1)
